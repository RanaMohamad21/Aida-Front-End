...
function SettingsPage() {
    ...
    const handleSaveSettings = () => {
      ...
      fetch('/api/save-settings', {
        ...
        body: JSON.stringify({ subscribedProducts, cartReminders, personalizedExperience }),
      ...
      <div>
        <div className="self-stretch flex flex-row items-start justify-start max-w-full">
          <div className="flex-1 flex flex-col items-start justify-between py-10 px-20 box-border min-h-[615px] mq750:py-[26px] mq750:px-10 mq750:box-border">
                ...
                <div className="flex-1 flex flex-col items-start justify-start gap-[16px] ">
                  <div className="self-stretch flex flex-col items-start justify-start gap-[16px] ">
                    <div className="self-stretch flex flex-row items-center justify-between gap-[20px] mq450:flex-wrap">
                      ...
                      <div 
                        className={`relative w-12 h-7 rounded-full cursor-pointer ${subscribedProducts ? 'bg-darkturquoise border border-teal ' : 'bg-gray border border-darkGray border-opacity-35 '}`} 
                        onClick={() => setSubscribedProducts(!subscribedProducts)}
                      >
                        <div className={`absolute top-[2px] ${subscribedProducts ? 'left-[24px]' : 'left-[2px]'} shadow-[0px_2px_4px_rgba(39,_39,_39,_0.1)] rounded-full bg-white w-6 h-6 transition-all`} />
                      </div>
                    </div>
                    <div className="relative text-base text-black inline-block ">
                      You&apos; receive emails when there&apos;s when there&apos;s a restock or when discounts are available
                    </div>
                  </div>
                  <div className="self-stretch flex flex-row items-center justify-between gap-[14px] mq400:flex-wrap">
                    ...
                    <div 
                      className={`relative w-12 h-7 rounded-full cursor-pointer ${cartReminders ? 'bg-darkturquoise border border-teal ' : 'bg-gray border border-darkGray border-opacity-35 '}`} 
                      onClick={() => setCartReminders(!cartReminders)}
                    >
                      <div className={`absolute top-[2px] ${cartReminders ? 'left-[24px]' : 'left-[2px]'} shadow-[0px_2px_4px_rgba(39,_39,_39,_0.1)] rounded-full bg-white w-6 h-6 transition-all`} />
                    </div>
                  </div>
                ...
                <div className="flex-1 flex flex-col items-start justify-start gap-[16px] ">
                  <div className="self-stretch flex flex-col items-start justify-start gap-[16px] ">
                    <div className="self-stretch flex flex-row items-center justify-between gap-[20px] mq450:flex-wrap">
                      ...
                      <div 
                        className={`relative w-12 h-7 rounded-full cursor-pointer ${personalizedExperience ? 'bg-darkturquoise border border-teal ' : 'bg-gray border border-darkGray border-opacity-35'}`} 
                        onClick={() => setPersonalizedExperience(!personalizedExperience)}
                      >
                        <div className={`absolute top-[2px] ${personalizedExperience? 'left-[24px]' : 'left-[2px]'} shadow-[0px_2px_4px_rgba(39,_39,_39,_0.1)] rounded-full bg-white  w-6 h-6 transition-all`} />
                      </div>
                    </div>
                    <div className="relative text-base text-black inline-block ">
            ...
            <div onClick={()=>handleLogout()} className="cursor-pointer [border:none] py-4 px-4 bg-darkturquoise rounded-lg flex flex-row items-start justify-center whitespace-nowrap hover:bg-lightseagreen">
            ...
            <div className="self-stretch flex flex-row items-center justify-start pt-[20px] pb-0 pl-0 gap-[15px] text-xl text-darkturquoise  mt-5 font-roboto border-t-[1px] border-solid border-darkturquoise lg:pr-[419px] lg:box-border mq750:flex-wrap mq750:pl-5 mq750:pr-5 mq750:pb-5 mq750:box-border">
              <Link className="relative [text-decoration:underline] font-medium " to="/delete">
                Delete My Account
              </Link>
